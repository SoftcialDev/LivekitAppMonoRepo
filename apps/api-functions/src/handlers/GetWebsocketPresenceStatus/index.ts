...
import { ok, badRequest, unauthorized, forbidden } from '../../index';
    ...
    return withAuth(ctx, async () => {
      ...
      try {
        ...
        const items: PresenceItem[] = users.map((u: {
          ...
          azureAdObjectId: string;
          role: string;
          presence: { status: string; lastSeenAt: Date | null } | null;
          supervisor: { id: string; email: string; fullName: string | null } | null;
        }) => {
          const item = {
            email:           u.email,
            fullName:        u.fullName ?? "",
