...
import { CommandUserNotFoundError, CommandUserDeletedError, CommandInvalidUserRoleError, CommandNotFoundError } from '../errors/CommandErrors';
...
export class CommandAcknowledgmentService implements ICommandAcknowledgmentService {
  ...
  private async validatePsoRole(callerId: string): Promise<void> {
    ...
    if (!user) {
      throw new CommandUserNotFoundError('User not found');
    }

    if (user.deletedAt) {
      throw new CommandUserDeletedError('User is deleted');
    }

    if (user.role !== UserRole.PSO) {
      throw new CommandInvalidUserRoleError('Only PSOs may acknowledge commands');
    }
  }
  ...
  private async validateCommandIds(commandIds: string[]): Promise<void> {
    ...
    if (existingIds.length !== commandIds.length) {
      ...
      throw new CommandNotFoundError(`Command IDs not found: ${missingIds.join(', ')}`);
    }
  }
}
