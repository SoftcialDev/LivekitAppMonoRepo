...
import { ContactManagerUserNotFoundError, ContactManagerProfileNotFoundError } from '../errors/ContactManagerErrors';
/**
...
export class ContactManagerDomainService {
  ...
  async createContactManager(request: CreateContactManagerRequest): Promise<ContactManagerProfile> {
    ...
    if (!user) {
      throw new ContactManagerUserNotFoundError(`User with email "${normalizedEmail}" not found in database`);
    }
  ...
  async deleteContactManager(request: DeleteContactManagerRequest): Promise<void> {
    ...
    if (!profile) {
      throw new ContactManagerProfileNotFoundError(`Contact Manager profile "${request.profileId}" not found`);
    }
  ...
  async getMyContactManagerStatus(azureAdObjectId: string): Promise<ContactManagerStatusResponse> {
    ...
    if (!user) {
      throw new ContactManagerUserNotFoundError(`User with Azure AD Object ID "${azureAdObjectId}" not found`);
    }

    const profile = await this.userRepository.findContactManagerProfileByUserId(user.id);

    if (!profile) {
      throw new ContactManagerProfileNotFoundError(`Contact Manager profile for user "${user.email}" not found`);
    }
