...
import { SnapshotNotFoundError } from "../errors/SnapshotErrors";
...
export class DeleteSnapshotDomainService {
  ...
  async deleteSnapshot(request: DeleteSnapshotRequest): Promise<DeleteSnapshotResponse> {
    ...
    if (!snapshot) {
      throw new SnapshotNotFoundError(`Snapshot with ID ${request.snapshotId} not found`);
    }
    ...
    try {
      ...
      // Failed to delete blob - continue with database deletion
    }
