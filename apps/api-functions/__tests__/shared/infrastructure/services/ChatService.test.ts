...
describe('ChatService', () => {
  ...
  describe('getOrSyncChat', () => {
    it('should create a new chat if no record exists for Contact Managers', async () => {
      ...
      expect(mockGraphClient.post).toHaveBeenCalledWith(expect.objectContaining({
        ...
        members: expect.arrayContaining([
          expect.objectContaining({ '@odata.type': '#microsoft.graph.chatMembers', 'roles': ['owner'], 'id': 'oid1' }),
          expect.objectContaining({ '@odata.type': '#microsoft.graph.chatMembers', 'roles': ['owner'], 'id': 'oid2' }),
        ]),
      }));
    ...
    it('should return existing chat ID if record exists for Contact Managers', async () => {
      ...
      expect(result).toBe(mockChatId);
      expect(mockPrisma.user.findMany).not.toHaveBeenCalled(); // Should not fetch users if chat exists
  ...
  describe('sendMessage', () => {
    ...
    it('should send a message to the chat', async () => {
      ...
      expect(mockGraphClient.post).toHaveBeenCalledWith(expect.objectContaining({
        '@odata.type': '#microsoft.graph.chatMembers',
        'roles': ['owner'],
      ...
      expect(mockGraphClient.post).toHaveBeenCalledWith(expect.objectContaining({
        body: {
          contentType: 'html',
          content: expect.stringContaining(mockMessage.subject),
        },
      }));
    });
    ...
    it('should throw an error if sending message fails', async () => {
      ...
      await expect(chatService.sendMessage(mockToken, mockChatId, mockMessage)).rejects.toThrow('Failed to send chat message: Message send error');
    });
    ...
    it('should handle message with image URL', async () => {
      ...
      expect(mockGraphClient.post).toHaveBeenCalledWith(expect.objectContaining({
        body: {
          contentType: 'html',
          content: expect.stringContaining(`<img src="${messageWithImage.imageUrl}"`),
        },
      }));
    });
  });
