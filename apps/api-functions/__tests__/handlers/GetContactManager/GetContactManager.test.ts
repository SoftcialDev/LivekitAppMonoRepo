/**
 * @fileoverview GetContactManager handler - unit tests
...
async function runHandler(ctx: Context, req: HttpRequest) {
  const handler = (await import('../../../GetContactManager/index')).default;
  return handler(ctx, req);
}

describe('GetContactManager handler - unit', () => {
  let ctx: Context;
  let req: HttpRequest;
  ...
  beforeEach(() => {
    ...
    appService = { listContactManagers: jest.fn() };
  ...
  it('should return 200 with contact managers list', async () => {
    const payload = { contactManagers: [{ id: 'cm-1' }, { id: 'cm-2' }] };
    appService.listContactManagers.mockResolvedValue(toPayloadify(payload));
    ...
    expect(appService.listContactManagers).toHaveBeenCalledWith(ctx.bindings.callerId);
  ...
  it('should return 500 when application service throws', async () => {
    appService.listContactManagers.mockRejectedValue(new Error('Service error'));
    ...
    expect(ctx.res).toEqual({
      status: 500,
      headers: { 'Content-Type': 'application/json' },
      body: { error: 'Failed to fetch Contact Managers' }
    });
  });
  ...
  it('should handle null payload with 200', async () => {
    appService.listContactManagers.mockResolvedValue({ toPayload: () => null } as any);
  ...
  it('should handle undefined payload with 200', async () => {
    appService.listContactManagers.mockResolvedValue({ toPayload: () => undefined } as any);
