/**
 * @fileoverview OnWebsocketConnected handler - unit tests
 * @summary Unit tests for OnWebsocketConnected Azure Function handler
...
async function runHandler(ctx: Context) {
  const handler = (await import('../../../OnWebsocketConnected/index')).default;
  return handler(ctx);
}

describe('OnWebsocketConnected handler - unit', () => {
  let ctx: Context;
