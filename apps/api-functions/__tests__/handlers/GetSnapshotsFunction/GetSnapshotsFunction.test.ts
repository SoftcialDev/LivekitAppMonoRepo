/**
 * @fileoverview GetSnapshotsFunction handler - unit tests
...
async function runHandler(ctx: Context, req: HttpRequest) {
  const handler = (await import('../../../GetSnapshotsFunction/index')).default;
  return handler(ctx, req);
}

describe('GetSnapshotsFunction handler - unit', () => {
  let ctx: Context;
  let req: HttpRequest;
  ...
  beforeEach(() => {
    ...
    appService = { getSnapshots: jest.fn() };

    const initialize = jest.fn();
    const resolve = jest.fn((token: string) => {
      if (token === 'GetSnapshotsApplicationService') return appService;
      return null;
    });
    ...
    jest.doMock('../../../shared/domain/value-objects/GetSnapshotsRequest', () => ({
      GetSnapshotsRequest: { fromCallerId: (id: string) => ({ callerId: id }) }
    }));
  });

  it('should return 200 with snapshots payload', async () => {
    const payload = { reports: [{ id: 'r1' }] };
    appService.getSnapshots.mockResolvedValue(toPayloadify(payload));

    await runHandler(ctx, req);

    expect(container.initialize).toHaveBeenCalled();
    expect(container.resolve).toHaveBeenCalledWith('GetSnapshotsApplicationService');
    expect(appService.getSnapshots).toHaveBeenCalledWith(
      ctx.bindings.callerId,
      expect.objectContaining({ callerId: ctx.bindings.callerId })
    );
  ...
  it('should return 500 when application service throws', async () => {
    appService.getSnapshots.mockRejectedValue(new Error('DB error'));
    ...
    expect(ctx.res).toEqual({
      status: 500,
      headers: { 'Content-Type': 'application/json' },
      body: { error: 'Internal error fetching snapshots' }
    });
  });
  ...
  it('should handle null response payload with 200', async () => {
    appService.getSnapshots.mockResolvedValue({ toPayload: () => null } as any);
  ...
  it('should handle undefined response payload with 200', async () => {
    appService.getSnapshots.mockResolvedValue({ toPayload: () => undefined } as any);
