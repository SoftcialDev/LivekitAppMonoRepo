/**
 * @fileoverview GetLivekitRecordings handler - unit tests
...
async function runHandler(ctx: Context, req: HttpRequest) {
  const handler = (await import('../../../GetLivekitRecordings/index')).default;
  return handler(ctx, req);
}

describe('GetLivekitRecordings handler - unit', () => {
  let ctx: Context;
  let req: HttpRequest;
  ...
  beforeEach(() => {
    ...
    (req as any).query = { page: 1, pageSize: 10 };
    ctx.req = req;
    ctx.bindings.callerId = TestHelpers.generateUuid();

    appService = { getLivekitRecordings: jest.fn() };

    const initialize = jest.fn();
    const resolve = jest.fn((token: string) => {
      if (token === 'GetLivekitRecordingsApplicationService') return appService;
      return null;
    });
    ...
    jest.doMock('../../../shared/domain/value-objects/GetLivekitRecordingsRequest', () => ({
      GetLivekitRecordingsRequest: { fromQuery: (q: any) => q }
    }));
  });

  it('should return 200 with recordings payload', async () => {
    const payload = { items: [], count: 0 };
    appService.getLivekitRecordings.mockResolvedValue(toPayloadify(payload));

    await runHandler(ctx, req);

    expect(container.initialize).toHaveBeenCalled();
    expect(container.resolve).toHaveBeenCalledWith('GetLivekitRecordingsApplicationService');
    expect(appService.getLivekitRecordings).toHaveBeenCalledWith(
  ...
  it('should return 500 when application service throws', async () => {
    appService.getLivekitRecordings.mockRejectedValue(new Error('DB error'));
  ...
  it('should handle null response payload with 200', async () => {
    appService.getLivekitRecordings.mockResolvedValue({ toPayload: () => null } as any);
  ...
  it('should handle undefined response payload with 200', async () => {
    appService.getLivekitRecordings.mockResolvedValue({ toPayload: () => undefined } as any);
