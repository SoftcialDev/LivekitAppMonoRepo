...
import { ISnapshotReasonRepository, SnapshotReason } from '../../domain/interfaces/ISnapshotReasonRepository';
...
export class SnapshotReasonRepository implements ISnapshotReasonRepository {
   ...
   */
  async findAllActive(): Promise<SnapshotReason[]> {
   ...
   */
  async findById(id: string): Promise<SnapshotReason | null> {
   ...
   * @returns Promise that resolves to the snapshot reason or null
   */
  async findByCode(code: string): Promise<SnapshotReason | null> {
    const reason = await prisma.snapshotReason.findUnique({
      where: { code }
    });
