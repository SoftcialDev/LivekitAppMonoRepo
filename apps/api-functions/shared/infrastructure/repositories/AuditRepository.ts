...
export class AuditRepository implements IAuditRepository {
  ...
  async findByEntity(entity: string, entityId: string): Promise<AuditLog[]> {
    try {
      ...
      throw new Error(`Failed to find audit logs by entity: ${error.message}`);
    }
  }
  ...
  async findByUser(changedById: string): Promise<AuditLog[]> {
    try {
      ...
      throw new Error(`Failed to find audit logs by user: ${error.message}`);
    }
  }
  ...
  async findByDateRange(startDate: Date, endDate: Date): Promise<AuditLog[]> {
    try {
      ...
      throw new Error(`Failed to find audit logs by date range: ${error.message}`);
    }
  }
}
