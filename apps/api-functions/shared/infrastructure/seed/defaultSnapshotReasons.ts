...
export async function seedDefaultSnapshotReasons(): Promise<void> {
  console.log('[Seed] Starting snapshot reasons seed...');
  ...
  for (const reason of DEFAULT_REASONS) {
    ...
    if (!existing) {
      ...
      console.log(`[Seed] Created snapshot reason: ${reason.label}`);
    } else {
      console.log(`[Seed] Snapshot reason already exists: ${reason.label}`);
    }
  }

  console.log('[Seed] Snapshot reasons seed completed');
}
